...
export default function ReportAll(props) {
        ...
        if (data.length > 0) {
            ...
            data.map((tr, index) => {
                tr.items.forEach((item) => {
                    temp.push([
                        tr.id, moment(tr.createdAt).format('YYYY-MM-DD HH:mm:ss'),
                        tr.paymentType,
                        tr.transactionType,
                        item.name,
                        item.productCode,
    ...
    const headerData = [
        "Transaksi Id", "Tanggal", "Pembayaran", "Sumber Transaksi", "Nama Produk", "Kode Produk", "Kuantitas", "Harga Jual", "Harga Modal", "Untung", "Total Harga"
    ]
    ...
    const saveDataExcell = async () => {
        try {
            ...
            var path = RNFS.DownloadDirectoryPath + `/omorfo-report-all-from${moment(startDate).format('YYYY-MM-DD')}to${moment(endDate).format('YYYY-MM-DD')}.xlsx`;
    ...
    const saveDataPdf = async () => {
        ...
        let options = {
            html: template,
            fileName: `omorfo-report-all-from${moment(startDate).format('YYYY-MM-DD')}to${moment(endDate).format('YYYY-MM-DD')}`,
            directory: 'Download',
        };
